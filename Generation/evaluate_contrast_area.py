...
def load_png_images(folder_path, num_images=200):
    """
    从指定文件夹加载PNG图像 (generated_1.png 到 generated_{num_images}.png)
    返回形状为 [num_images, H, W, 3] 的 uint8 tensor
    """
    images = []
    for i in range(1, num_images + 1):
        img_path = os.path.join(folder_path, f'generated_{i}.png')
        img = Image.open(img_path).convert('RGB')
        images.append(np.array(img))
    
    # 转换为 tensor [B, H, W, 3]
    return torch.tensor(np.array(images), dtype=torch.uint8)


def evaluate_metrics(all_gen_images, all_gt_images, device=device):

    # Load all tensors to GPU - this may cause OOM if the tensors are large
    all_gt_images = all_gt_images.float().to(device)
    all_gen_images = all_gen_images.float().to(device)
    ...
    all_gen_images = clamp(all_gen_images, 0.0, 1.0)
    all_gt_images = clamp(all_gt_images, 0.0, 1.0)
    ...
    del inception_model, real_features, gen_features, all_gen_images_fid, all_gt_images_fid
    ...
    return{
            ...
            "SSIM": ssim_value,
            "FID Score": fid_score,
            "AlexNet(2)": alexnet2_sim,
            "AlexNet(5)": alexnet5_sim,
            "IS(Mean)": is_mean,
            "IS(Std)": is_std,
            "CLIPSim": mean_score
    }
...
all_gt_images = torch.load(gt_path)  # [200, 500, 500, 3] uint8

# ======================== 遍历所有生成图像目录 ========================
base_dir = '/path/to/EEG2Img-Contrastive-Decoding/eeg_image_reconstruction_results/contra_area_EEG/'
results_summary = []

# 获取所有符合条件的文件夹
folders = [f for f in os.listdir(base_dir) 
           if os.path.isdir(os.path.join(base_dir, f)) 
           and f.startswith('contra_')]

for folder in folders:
    gen_folder = os.path.join(base_dir, folder, 'onlyCLIP-pipeline')
    ...
    # 加载生成图像
    gen_images = load_png_images(gen_folder)
    
    # 验证图像数量
    if gen_images.shape[0] != 200:
        print(f"Warning: {folder} has {gen_images.shape[0]} images (expected 200). Skipping.")
        continue
    
    # 执行评估
    metrics = evaluate_metrics(gen_images, all_gt_images)
    ...
    result_csv = os.path.join(gen_folder, f"{folder}_results.csv")
...
if results_summary:
    ...
    # 重排列顺序
    cols = ["folder"] + [col for col in summary_df.columns if col != "folder"]
    summary_df = summary_df[cols]
    summary_csv = "/path/to/EEG2Img-Contrastive-Decoding/results_summary_contra_area_EEG.csv"
