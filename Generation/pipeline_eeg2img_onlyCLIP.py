...
def eeg_to_image_pipeline(eeg_test_data, subject_id, output_dir=None, device=None, seed=42):
    ...
    print(f"eeg_features[0]: {eeg_features[0][:100]}")  # 打印第一个样本的EEG特征
    exit()
    # 保存EEG特征
    ...
    for i in range(num_samples):
        ...
        # 清理显存
        del batch_clip_features, batch_eeg_features
        torch.cuda.empty_cache()
    # 合并所有结果
    ...
    # 清理显存
    del diffusion_prior, pipe, all_clip_features, eeg_features
    torch.cuda.empty_cache()
